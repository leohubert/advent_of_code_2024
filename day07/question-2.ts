 ...
 * - Multiplication (*)
 *
 * The function will try to add `||` to resolve the equation, this combine the results of the two part of the equation like example below:
 *: - `1 + 2 + 3 * 4` will be `1 + 2  || 3 * 4` = 212
 *  - `1 + 2 * 3` will be `1 + 2 || 3` = 23
 */
function findEquations(result: number, numbers: number[]): string[] {
    ...
    function helper(currentResult: number, currentEquation: string, index: number) {
        ...
        helper(Number(`${currentResult}${numbers[index]}`), `${currentEquation} || ${numbers[index]}`, index + 1);
    }
